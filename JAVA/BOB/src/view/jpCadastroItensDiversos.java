 ...
 * @author mario
 */
public class jpCadastroItensDiversos extends javax.swing.JPanel {
    ...
    private void AtualizarPai(){
        try{
            switch (tipoPedido){
                    case Pedido.OLEOFILTRO:{
                        jfTrocaOleo jf = (jfTrocaOleo)jpPai;
                        jf.atualizarTela();
                    }
            }
        }catch(Exception e){
    ...
    public jpCadastroItensDiversos(Object jpPai, int tipoPedido, Item item){
        inicializar();
        try{
            this.tipoPedido = tipoPedido;
            switch (tipoPedido){
                case Pedido.OLEOFILTRO:{
                    this.jpPai = (jfTrocaOleo) jpPai;
                }
            }
                        
            int i = -1;
            i = tipos[item.getTipo()];
            if(i>=0){
                cbbTipo.setSelectedIndex(i);
                int j =0, c =0;
                for(Item it: arraylist){
                    if(it.getId()==item.getId()){
                        jList.setSelectedIndex((arraylist.size()-arraylist.indexOf(it))-1);
                    }
                  //  c++;
                }
                //jList.setSelectedIndex(c);
            }
        }catch(Exception e){
    ...
    private void preencherCbbTipo(){
        ...
        for(String tipoS: Item.TIPO_STRING){
            cbbTipo.addItem(tipoS);
        }
        tipos = Item.TIPOS_INT;
    }
    ...
    private void preencherSelectedListview(){
        try{
            ...
            if(j>=0){
                ...
                cbbTipo.setSelectedIndex(arraylist.get(i).getTipo());
    ...
    private void preencherListView(int[] itensTipo){
        DefaultListModel listModel = new DefaultListModel();
        int j=0;
//        arraylist = new ItemControl().getArrayListItem();
       
       arraylist = new ItemControl().getArrayListItemByTipo(itensTipo);
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Cadastro de Itens Diversos");
    ...
    private void AtualizarTela(){
            limparCampos();
            preencherListView(new int[]{tipos[cbbTipo.getSelectedIndex()]});
            AtualizarPai();
    }
