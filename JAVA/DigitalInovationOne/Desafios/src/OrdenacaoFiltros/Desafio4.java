...
public class Desafio4 {
    public static void main(String[] args) {
        ...
        int nTeste = Integer.parseInt(scanner.nextLine());

        try{

            for(int teste = 0; teste <nTeste; teste++){
                int nClientes = Integer.parseInt(scanner.nextLine());
                String valores = scanner.nextLine();
                List<String> clientList = Arrays.asList(valores.split(" "));

                List<String> clientListOrdenada =
                        clientList.stream()
                                .sorted((o1, o2) -> {return Integer.parseInt(o2) - Integer.parseInt(o1);})
                                .collect(Collectors.toList());
                int nClientesNaPosicaoCorreta = 0;
                for(int i = 0; i< clientList.size(); i++){
                    if(clientListOrdenada.get(i)==clientList.get(i)){
                        nClientesNaPosicaoCorreta++;
                    }
                }
                System.out.println(nClientesNaPosicaoCorreta);
...
*   uma senha numérica em seu aparelho de celular via sms e que
*   a ordem da fila será dada não pela ordem de chegada,
...
* contendo um inteiro, indicando o número de clientes que não precisaram trocar de lugar
